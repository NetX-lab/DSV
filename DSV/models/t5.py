...
# adapted from https://github.com/PixArt-alpha/PixArt-alpha/blob/master/diffusion/model/t5.py
    ...
    def __init__(
        ...
        *,
        ...
        if local_cache:
            ...
            cache_dir = "/mnt/xtan-jfs/t5-v1_1-xxl"
            for filename in [
                "config.json",
                "special_tokens_map.json",
                "spiece.model",
...
def get_gpu_memory():
    if torch.cuda.is_available():
        for i in range(torch.cuda.device_count()):
            memory = torch.cuda.get_device_properties(i).total_memory / 1024**2
...
if __name__ == "__main__":
    t5 = T5Embedder(
        dir_or_name="t5-v1_1-xxl", device="cuda", torch_dtype=torch.bfloat16
    )
    ...
    with torch.no_grad():
        output = t5.get_text_embeddings(text_prompt)
    ...
    with profile(
        ...
        for i in range(5):
            with record_function("t5_get_text_embeddings"):
                t5.get_text_embeddings(text_prompt)

    # prof.export_chrome_trace("t5.json")
