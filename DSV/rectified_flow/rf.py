...
def timestep_transform(
    ...
    # NOTE: Here our VAE does not downsample the frames, so we use the original num_frames
...
class RFlowScheduler:
    ...
    def training_losses(
        ...
        # print(f"model_output: {model_output.shape}; x_start: {x_start.shape}; noise: {noise.shape}")
        velocity_pred = model_output.chunk(2, dim=1)[0]
        # print(f"model_output: {model_output.shape}; velocity_pred: {velocity_pred.shape}; x_start: {x_start.shape}; noise: {noise.shape}")
        if weights is None:
