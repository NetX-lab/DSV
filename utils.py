...
def cp_tp_region_split_input(input_):
    if global_variable.CP_TP_MESH == None:
        return input_

    rank_in_cp_tp_group = dist.get_rank(global_variable.CP_TP_MESH.get_group())
    cp_tp_group_size = dist.get_world_size(global_variable.CP_TP_MESH.get_group())

    # input: b,f,c,h,w
    b, f, c, h, w = input_.shape

    assert (
        f % cp_tp_group_size == 0
    ), f"f:{f} must be divisible by cp_tp_group_size:{cp_tp_group_size}"
    ...
    input_ = input_[
        ...
        rank_in_cp_tp_group
        * f
        // cp_tp_group_size : (rank_in_cp_tp_group + 1)
        * f
        // cp_tp_group_size,
        ...,
    ]
...
def clip_grad_norm_(
    ...
    # skip param low_rank_qk_proj

    # parameters is a list of named_parameters

    grads = [
        p.grad
        for name, p in parameters
        if p.grad is not None and "low_rank_qk_proj" not in name
    ]
...
def update_ema(ema_model, model, decay=0.9999):
    ...
    for name, buffer in model_buffers.items():
        ema_buffers[name].copy_(buffer.to(torch.float32))
...
def find_model(
    model_name, ckpt_name=None, use_gpu=False, gpu_id=0, return_full_ckpt=False
):
    """
    ...
    if use_gpu:
        checkpoint = torch.load(
            model_name, map_location=lambda storage, loc: storage.cuda(gpu_id)
        )
    else:
    ...
    if ckpt_name != None:
        ...
        assert ckpt_name in checkpoint
