...
def main(args):
    ...
    global_variable.LOW_RANK_DICT = args.low_rank_dict
    ...
    if global_variable.TEST_LARGE_SCALE == False:
        low_rank_module = (
            LowRankModule(
                ...
                inner_dim=args.num_heads * args.head_dim,
                low_rank_dim=16 * args.num_heads,
    ...
    if args.dataset not in ["webvid"]:
        ...
        if global_variable.CONTEXT_PARALLEL_GROUP != None:
            ...
            sampler_rank = rank // dist.get_world_size(
                global_variable.CONTEXT_PARALLEL_GROUP
            )
        ...
        for epoch in range(first_epoch, num_train_epochs):
            ...
            for step, video_data in enumerate(loader):
                    ...
                    if args.dataset not in ["webvid"] and args.extras != 78:
                        ...
                        if args.dataset in ["ucf101_img", "videogen", "openvid",'dummy']:
                            ...
                            if (
                                ...
                                print(
                                    f"rank:{rank}; step:{step}; input video shape for this rank after split:{x.shape}; text_encoder_hidden_states shape:{text_encoder_hidden_states.shape}; encoder_attention_mask shape:{encoder_attention_mask.shape}"
                                )
                ...
                if (
                    ...
                    torch.cuda.synchronize()
                    grads = get_lr_grad_list(low_rank_module)
                ...
                if (
                    ...
                    if rank == 0:
                        checkpoint["low_rank_module"] = low_rank_module.state_dict()
                        checkpoint["low_rank_opt"] = low_rank_opt.state_dict()
